...
*!*						REPLACE cImpSisColegio.ano			WITH MATRICULA.ano		IN 'cImpSisColegio'
					...
					Replace cImpSisColegio.e_nome 		WITH COOPER_EMP.razao				,;
							...
							cImpSisColegio.e_endereco 	WITH ALLTRIM(COOPER_EMP.endereco) + " nº " + ALLTRIM(COOPER_EMP.numero)	,;
...
*!*								REPLACE cImpSisColegio.mes&xvenc.		WITH xMes	,;
			...
			IF RECCOUNT('cImpSisColegio')!= 0
				xFILTRO = ""	
				&& ROTINA PARA CARREGAR OS DADOS PADRÕES DO CABEÇALHO
				...
				objVisRelatorio.set_email_assunto( "Matrícula aluno " + cImpSisColegio.a_nome + " (" + TTOC(DATETIME()) + ")" )		
				
				&& APENAS EXECUTA O RELATÓRIO
			...
			ELSE 
				WaitEnter("Não há dados disponiveis!")
			ENDIF
			fechalivre("CURSO") 
...
*!*	     IF EMPTY( ImpEscolha )
*!*		    WaitEnter("Atenção! Nenhuma impressora definida para impressão..." )
*!*		    RETURN -1
*!*		 ENDIF 
...
FUNCTION RL_MA01(pAno)
	...
	DO WHILE !EOF('cDados')
		...
		IF cDados.vl_parcela > 0	&& CRIADO DEPOIS DAS ALTERAÇÕES DE 11/2021
	...
	IF RECCOUNT('cImpSisColegio')!= 0
		...
		Replace ALL cImpSisColegio.cabec_titulo		WITH xTitulo		,;
					...
					cImpSisColegio.cabec_end3		WITH xEnd3			,;
					cImpSisColegio.cabec_logo		WITH xLogo			,;
					cImpSisColegio.cabec_rel		WITH "Relatório de Alunos matriculados (financeiro)"				,;
					cImpSisColegio.cabec_filtro		WITH xFILTRO					,;
					cImpSisColegio.filtro			WITH xFILTRO		,;
					cImpSisColegio.rodape1			WITH xRodape1		,;
					cImpSisColegio.rodape2			WITH xRodape2		IN "cImpSisColegio"
		...
		objVisRelatorio.set_email_assunto( "Relatório de Matrículas " + " (" + TTOC(DATETIME()) + ")" )		
...
*!*			ENDIF
	...
	ELSE 
		WaitEnter("Não a Dados disponiveis!")
	ENDIF 
...
*!*				xCondicao = C_PAGTO.descricao && ARMAZENO A CONDIÇÃO DE PAGAMENTO
...
*Relatório de Alunos matriculados no Curso Anglo Jundiaí
	...
	IF RECCOUNT('cImpSisColegio')!= 0
		...
		Replace ALL cImpSisColegio.cabec_titulo		WITH xTitulo		,;
					...
					cImpSisColegio.cabec_end3		WITH xEnd3			,;
					cImpSisColegio.cabec_logo		WITH xLogo			,;
					cImpSisColegio.cabec_rel		WITH "Relatório de Alunos matriculados"				,;
					cImpSisColegio.cabec_filtro		WITH xFILTRO					,;
					cImpSisColegio.filtro			WITH xFILTRO		,;
					cImpSisColegio.rodape1			WITH xRodape1		,;
					cImpSisColegio.rodape2			WITH xRodape2		IN "cImpSisColegio"
...
*!*			ENDIF
	...
	ELSE 
		WaitEnter("Dados não localizados!" )
	ENDIF 
...
FUNCTION RL_MEN01(pTurma, pMes, pAno, pLocal, pData)
	...
	&& Definição da data // EU NAO CONSEGUI FAZER ISSO FUNCIONAR.. 
	&& SET DATE TO DMY 
...
*!*		ENDIF
	...
	xTitulo_REL = ""
	xTitulo_REL="Relatório de Mensalidades"
	IF pLocal == 1 AND pData == 2
		xTitulo_REL="Relatório de Mensalidades realizadas no Banco"
	ENDIF
	...
	&& POSICIONO O CURSOR NA DATA INICIAL... PELO INDICE ESCOLHIDO CONFORME AS OPÇÕES DA TELA(INDVECS OU INDPAGTO)
	SET NEAR ON
	=SEEK(DTOS(xDataIni), "V_ALUNO", "&xIndice.")

	DO WHILE &xPesquisa. and !EOF("V_ALUNO")
		&&PULO UM REGISTRO SE ELE ESTIVER MARCADO PARA DELEÇÃO OU CODIGO = 'NOVO'
		IF DELETED('V_ALUNO') OR ALLTRIM(V_ALUNO.codigo) = 'NOVO' OR EMPTY(V_ALUNO.codigo)
			SKIP 1 IN 'V_ALUNO'
			LOOP 
		ENDIF 
		
		&&SE A VARIAVEL LOCAL ESTIVER VAZIA SIGNIFICA QUE FOI SELECIONADO A OPÇÃO "TODOS"
*!*			IF !EMPTY(xLocal)
...
*!*			ENDIF
		...
		&& Subtraio um por que no laço ele sempre ira contar um zero a mais, pois usei o xCont na função left tambem
		&& entao ela nao podeia começar como zero.
		
		xCont = xCont -1
		&& 10 é a quantidade de caracteres q o campo codigo possui..
		xQtd = 10 - xCont
...
*!*		REPLACE cImpSisColegio.valor 	WITH xTotal					IN 'cImpSisColegio'
	...
	&& SE O CURSOR cMen CONTER INFORMAÇÃO... SELECIONO A IMPRESSORA E IMPRIME O RELATORIO
	SELECT("cImpSisColegio")
	IF RECCOUNT('cImpSisColegio')!= 0
...
*!*			ENDIF
	...
	ELSE 
		WaitEnter("Dados não encontrados!")
	ENDIF 
...
*!*								REPLACE cAluno.mes&xvenc.		WITH xMes	,;
			...
			IF RECCOUNT('cAluno')!= 0
				...
				objVisRelatorio.set_email_assunto( "Matrícula aluno " + cAluno.nome + " (" + TTOC(DATETIME()) + ")" )		
