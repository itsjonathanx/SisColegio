...
&& O restante das definições já utilizadas, continuam da mesma forma.
    ...
    Public array aMenuCadAluno[1,3]
    xI = 0
    
    xI = xI + 1
   	aMenuCadAluno[xI,1] = 'cadastrar [A]lunos'
	aMenuCadAluno[xI,2] = ''
	aMenuCadAluno[xI,3] = 'FORM->frmalunocadastro'
			
	
	&& CRIA O MENU PADRAO 
	IF !pMenuConsulta
		xCriar = "Criar_Menu( @aMenuCadAluno )"
		&xCriar.
	ELSE
